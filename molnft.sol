...
contract MolNFT is ERC721Enumerable, Ownable {
    ...
    /**
     * @dev Because your environment wants an argument for Ownable, we do Ownable(msg.sender).
     */
    ...
    function mintNFT(
        ...
        // Emit event
        if (parentId == 0) {
            emit ParentNFTMinted(msg.sender, tokenId);
        } else {
            emit ChildNFTMinted(msg.sender, tokenId, parentId);
        }
    }
    ...
    function tokenExists(uint256 tokenId) public view returns (bool) {
        // The same approach you had in your working contract:
    ...
    {
        // Check existence using tokenExists()
    ...
    function _getFieldValue(string memory field, uint256 tokenId) internal view returns (string memory) {
        ...
        // imageBase64/fileBase64 are excluded from text search by design.
        return "";
    }
